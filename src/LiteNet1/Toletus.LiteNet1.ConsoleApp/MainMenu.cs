...
using Toletus.LiteNet2.ConsoleApp.LiteNet2;
...
namespace Toletus.LiteNet1.ConsoleApp
{
    internal static class MainMenu
    {
        ...
        {
            //Console.WriteLine($"{Environment.NewLine}LiteNet2 Response: {responseCommand.Command} [{responseCommand.Payload.ToHexString(" ")}]");
            Console.WriteLine($"{Environment.NewLine}LiteNet2 Response: {responseCommand}]");

            if (responseCommand.Identification != null)
                Console.WriteLine($"{Environment.NewLine}LiteNet2 Identification: {responseCommand.Identification.Device} {responseCommand.Identification.Value} {responseCommand.Identification.Template}");
        }
    }
}